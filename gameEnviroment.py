...
BIRD_IMGS = [pygame.transform.scale2x(pygame.image.load(os.path.join("imgs","bird1.png"))), pygame.transform.scale2x(pygame.image.load(os.path.join("imgs","bird2.png"))), pygame.transform.scale2x(pygame.image.load(os.path.join("imgs","bird3.png")))]  #tu będą przechowywane obrzy ptaków, funkcja scale powiększa x2 wielkość ptaków przy pozostawieniu grafiki
...
class Bird():
    ...
    def jump(self):
        ...
        self.tick_count          #Sprawdza kiedy ostatnio skakaliśmy
...
class Pipe:
    ...
    def collide(self, bird):
        ...
        bottom_offset = (self.x - bird.x, self.bottom- round(bird.y))        #przsuwa się pozycja względem osi x i y dolnej rury

        b_poin = bird_mask.overlap(bottom_mask, bottom_offset)               #Pokazuje punkt kolizji, jeżeli nie ma to zwraca NONE
        t_poin = bird_mask.overlap(top_mask, bottom_offset)
...
class Enviroment():
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            if self.bird.y + self.bird.img.get_height() >= 730:  # Jeżeli ptak uderzy w ziemię to koniec gry
                gameOver = True
            ...
            elif gameOver == False and pipe.passed == True:
        ...
        return tu musi być tablica, reward,gameOver
...
def main():
    ...
    while run:
        ...
        if bird.y +bird.img.get_height() >=730:            #Jeżeli ptak uderzy w ziemię to koniec gry
            pass
